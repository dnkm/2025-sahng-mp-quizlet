...
const SUBTOPICS = {
  ...
  string,
  Array<{ name: string; gradeLevels: number[] }>
>;

export const migrations = new Migrations<DataModel>(components.migrations);
export const run = migrations.runner();

export const generateSubTopics = migrations.define({
  table: "topics",
  async migrateOne(ctx, doc) {
    let subtopics = SUBTOPICS[doc.name as keyof typeof SUBTOPICS];
    if (subtopics) {
      for (let subtopic of subtopics) {
        await ctx.db.insert("subTopics", {
          topicId: doc._id,
          name: subtopic.name,
          gradeLevels: subtopic.gradeLevels,
          numDecks: 0,
        });
      }
    }
